...
# --------------------------------------------------------------------
# In the real campaign Cloudflare observed the literal User-Agent
# "truffleHog" hitting a token-verification endpoint on 2025-08-09.
# That is TruffleHog's default UA when its detectors do live
# verification. GTIG lists four separate User-Agents for the campaign
# (Salesforce-Multi-Org-Fetcher/1.0, Salesforce-CLI/1.0,
# python-requests/2.32.4, Python/3.11 aiohttp/3.12.15). Detecting any of
# them needs a log carrying the User-Agent header, which this tenant does
# not produce.
#
