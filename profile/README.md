...
A financially motivated actor compromised the Salesloft Drift application between March and August 2025.
Mandiant tracks it as UNC6395, Cloudflare as GRUB1, and ShinyHunters claimed it publicly.
The actor used Drift's OAuth integration to mass-exfiltrate Salesforce data from downstream customers, Cloudflare included.

Initial access came through Salesloft's GitHub repositories between March and June 2025.
From there the actor pulled Drift's OAuth tokens out of Salesloft's AWS environment.

Operations against victim tenants ran from August 8 to August 20, 2025.
The actor issued reconnaissance SOQL queries, then used Bulk API 2.0 to exfiltrate Case, Account, Contact and User objects.

Stolen records were mined for embedded secrets (AWS access keys, Snowflake credentials, passwords, SSO and VPN URLs) to enable downstream compromise.
Salesloft revoked Drift OAuth and refresh tokens and removed Drift from the Salesforce AppExchange on August 20, 2025, containing the campaign.
...
ShinyHunters publicly claimed responsibility for the campaign but did not share private indicators that would confirm operational overlap with prior ShinyHunters activity.

Mandiant tracks the cluster under the uncategorized designation UNC6395, which signals the group has not yet been linked with confidence to a known named actor.
Cloudflare uses its own internal designation GRUB1 for activity observed against its tenant.

Treat all three names as referring to the same activity cluster for this campaign, while preserving distinction in case future reporting separates them.

---

## 3. Motivation and Objectives

The actor's behavior is consistent with financial motivation rather than espionage or disruption.

Operational goals observed across the three reports:
...
- Cross-sector.
- Any organization running Salesforce with the Salesloft Drift integration enabled was in scope.
- Confirmed and reported victims sit primarily in technology and SaaS (Cloudflare named publicly).
...
Targeting was opportunistic, scoped by the Salesloft Drift install base.

Per permiso.io, Drift's per-user OAuth model meant the token inherited whatever privilege the connecting user held.
Any Salesforce admin who connected the integration handed the actor admin-level access.
One upstream compromise multiplied across hundreds of tenants.
...
1. **Pre-staging (2025-03 to 2025-06):** Access to Salesloft's GitHub repositories. Persistence came from adding a guest user to the organization.
...
6. **Containment (2025-08-20):** Salesloft revoked all active Drift access and refresh tokens. Drift was removed from the Salesforce AppExchange.
...
- Drift uses a per-user OAuth token architecture: each user who connects the integration grants a token with the privileges of that user.

Where a Salesforce admin had connected Drift, the actor obtained admin-equivalent access. This produced hundreds of valid downstream tokens from a single upstream compromise.
...
- The legitimate Salesloft OAuth baseline comes from AWS IP ranges. The actor's testing from AWS (44.215.108.109) blended into it. The pivot to DigitalOcean and Tor broke the baseline and is the highest-signal infrastructure indicator.
- No evidence of bulletproof hosting. The actor used hyperscalers and reputable VPS providers for testing, then anonymizing infrastructure for exfiltration.
...
These are detection proposals drawn from the campaign, not shipped rules.
Every one needs Salesforce Shield, a Tor exit list, or DLP.
The Implemented column says which of them `../detections/sigma/` actually covers.

| Rule Name | Logic | Data Source | Severity | Implemented |
|---|---|---|---|---|
| Drift OAuth ASN deviation | Drift Connected App login where source ASN is not in established baseline (AWS) | Salesforce Event Monitoring (Login, Login:oauthrefreshtoken) | High | no |
| Bulk API job rapid deletion | BulkApi2 job DELETE within 5 minutes of job COMPLETE | Salesforce Event Monitoring (BulkApi2, ApiTotalUsage:DELETE) | High | no, needs Shield |
| Recon-then-bulk sequence | Same identity executes `/sobjects/` list, then `/describe/`, then `SELECT COUNT()`, then bulk export within 24h | Salesforce Event Monitoring (RestApi, UniqueQuery, BulkApi2) | High | partly. [R3](../detections/sigma/R3_T1087.004_salesforce_volume_enumeration.yml) covers the volume but not the sequence |
| Suspicious User-Agent against Salesforce | API request with User-Agent matching `Salesforce-Multi-Org-Fetcher/1.0`, `python-requests/2.32.4`, `aiohttp/3.12.15`, or `Salesforce-CLI/1.0` from non-sanctioned source | Salesforce Event Monitoring (RestApi, ApiTotalUsage) | High | no, needs Shield |
| Tor egress to Salesforce | Salesforce login from a known Tor exit node | Salesforce Event Monitoring + Tor exit list | Critical | no |
| Secret pattern in case body | New or modified Case body matches `AKIA[0-9A-Z]{16}`, `snowflakecomputing.com`, or password keyword density | Salesforce DLP / case scanning | Medium | no |
...
The greater risk is second-order: stolen secrets enable lateral movement into AWS accounts, Snowflake data warehouses, and Google Workspace tenants of every organization whose support records contained credential material.
Even organizations that fully rotated Drift OAuth tokens remain exposed
if embedded secrets in their Salesforce case data were not also rotated.
...
- **NHI compromise as an attack surface multiplier:** The chain ran from one human identity at Salesloft into machine identities trusted across hundreds of customer environments. Service-to-service trust deserves the scrutiny given to human SSO.
- **Vendor secrets storage is a frontline target:** Tokens in AWS Secrets Manager and SSM Parameter Store were the lever for the whole campaign. Secrets stores need tight access control and real monitoring.
- **Per-user OAuth designs concentrate admin risk:** An integration connected under an admin's identity inherits admin reach. Scoped service-account integrations reduce this.
